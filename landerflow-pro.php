 ...
 * Description: Professional landing page setup utility - Install free & premium plugins, import CartFlows templates
 * Version: 1.0.1
 * Author: Robiul Awal
...
define('LANDERFLOW_PRO_PREMIUM_DIR', LANDERFLOW_PRO_PLUGIN_DIR . 'premium-plugins/');
    ...
    class LanderFlow_Pro
    {
        ...
        public function enqueue_assets($hook)
        {
            ...
            wp_localize_script('lf-admin', 'lfData', [
                'ajax' => admin_url('admin-ajax.php'), 'nonce' => wp_create_nonce('lf_nonce'),
                'plugins' => $this->all_plugins, 'core' => $this->core_slugs,
                'premium_plugins' => $this->premium_plugins, 'funnels' => $this->funnel_templates,
                'cartflows_url' => admin_url('admin.php?page=cartflows'),
                'cartflows_import_url' => admin_url('admin.php?page=cartflows&tab=import'),
                'has_premium_folder' => file_exists(LANDERFLOW_PRO_PREMIUM_DIR),
                'texts' => [
                    ...
                    'all_plugins_ready' => __('All plugins installed!', 'landerflow-pro'), 'confirm_bulk' => __('Process selected plugins?', 'landerflow-pro'),
        ...
        public function ajax_activate()
        {
            ...
            $slug = sanitize_text_field($_POST['slug']);
            $file = $this->all_plugins[$slug]['file'] ?? null;
            if (!$file) { foreach ($this->premium_plugins as $pp) { if ($pp['id'] === $slug) { $file = $pp['plugin_file']; break; } } }
            if (!$file) wp_send_json_error('Invalid plugin');
        ...
        public function ajax_get_all_status()
        {
            ...
            foreach ($this->premium_plugins as $pp) {
                ...
                $status['premium'][$pp['id']] = [
                    'name' => $pp['name'], 'installed' => $this->is_installed($file), 'active' => is_plugin_active($file),
                    'zip_exists' => !empty($pp['zip_url']), 'zip_size' => 'CDN'
                ];
            }
            wp_send_json_success($status);
        }
        ...
        public function ajax_premium_install()
        {
            ...
            $plugin_data = null;
            foreach ($this->premium_plugins as $pp) { if ($pp['id'] === $plugin_id) { $plugin_data = $pp; break; } }
            ...
            @unlink($zip_file); // Cleanup temp file
        ...
        /**
         * Download ZIP from CDN/GitHub to temp folder
         */
        private function download_from_cdn($plugin_data)
        {
            $zip_url = $plugin_data['zip_url'];
            if (empty($zip_url)) {
                return new WP_Error('no_url', 'No download URL for ' . $plugin_data['name']);
            }

            // Temp directory
            $temp_dir = LANDERFLOW_PRO_PLUGIN_DIR . 'temp/';
            if (!file_exists($temp_dir)) wp_mkdir_p($temp_dir);
            if (!is_writable($temp_dir)) return new WP_Error('no_write', 'Temp folder not writable');

            $filename = sanitize_file_name($plugin_data['id'] . '-' . time() . '.zip');
            ...
            if ($http_code !== 200) {
                @unlink($temp_file);
                return new WP_Error('http_error', 'HTTP ' . $http_code . ' for ' . $plugin_data['name']);
            }

            if (!file_exists($temp_file) || filesize($temp_file) < 500) {
                @unlink($temp_file);
                return new WP_Error('empty_file', 'Downloaded file is too small for ' . $plugin_data['name']);
            }
        ...
        public function ajax_download_json()
        {
            ...
            if (empty($json_url)) wp_send_json_error('Invalid URL');
            ...
            if (empty($json_body)) wp_send_json_error('Empty response');
            ...
            wp_send_json_success([
                ...
                'size' => size_format(strlen($json_body))
            ]);
        }
            ...
            <div class="lfp-wrap <?php echo $auto ? 'auto-install' : ''; ?>">
                <aside class="lfp-sidebar">
                    ...
                    <nav class="lfp-nav">
                        ...
                        <a href="#" class="lfp-nav-item soon"><span class="lfp-nav-icon">🎨</span><span>Customizer</span><span class="lfp-badge-soon">Soon</span></a>
                    </nav>
                    <div class="lfp-sidebar-footer"><div class="lfp-version-badge">v<?php echo LANDERFLOW_PRO_VERSION; ?></div><p>BY CodeOrbitStudio</p></div>
                </aside>
                <main class="lfp-main">
                    <!-- SETUP WIZARD -->
                    <section class="lfp-section" id="section-setup">
                        <div class="lfp-topbar"><div class="lfp-topbar-left"><h1 class="lfp-page-title">Setup Wizard</h1><p class="lfp-page-sub">Install free plugins from WordPress.org</p></div><div class="lfp-topbar-right"><div class="lfp-stat"><span class="lfp-stat-val stat-completed">0</span><span class="lfp-stat-lbl">Active</span></div><div class="lfp-stat-sep">/</div><div class="lfp-stat"><span class="lfp-stat-val stat-total"><?php echo count($this->core_slugs); ?></span><span class="lfp-stat-lbl">Core</span></div></div></div>
                    ...
                    <section class="lfp-section hidden" id="section-premium">
                        <div class="lfp-topbar"><div class="lfp-topbar-left"><h1 class="lfp-page-title">💎 Premium Plugins</h1><p class="lfp-page-sub">Auto-download & install from CDN. Source: <code>GitHub Releases</code></p></div><div class="lfp-topbar-right"><button type="button" id="lf-premium-install-all" class="lfp-btn lfp-btn-primary"><span>⬇</span> Install All (<?php echo $prem_count; ?>)</button><button type="button" id="lf-premium-refresh" class="lfp-btn lfp-btn-ghost"><span>↻</span> Refresh</button></div></div>
                            ...
                            <?php foreach ($this->premium_plugins as $pp): 
                                ...
                                <div class="lfp-plugin-card <?php echo $active?'lfp-active':($installed?'lfp-pending':'lfp-premium-ready'); ?>" data-plugin="<?php echo esc_attr($pp['id']); ?>" data-type="premium" data-core="<?php echo $pp['required']?'1':'0'; ?>">
                                    ...
                                    <div class="lfp-plugin-status-col">
                                        <div class="lfp-status-pill <?php echo $active?'active':($installed?'pending':'premium'); ?>" id="pill-<?php echo esc_attr($pp['id']); ?>"><span class="lfp-pill-dot"></span><span class="lfp-pill-text"><?php echo $active?'Active':($installed?'Inactive':'Install from CDN'); ?></span></div>
                    ...
                    <!-- TEMPLATES -->
                    <?php if ($cf_active): ?>
                    <section class="lfp-section hidden" id="section-templates"><?php $this->render_template_library(); ?></section>
                    <?php endif; ?>
        ...
        private function render_plugin_card($slug, $is_core = true) {
            $p = $this->all_plugins[$slug]; $in = $this->is_installed($p['file']); $ac = is_plugin_active($p['file']);
            ?>
        ...
        private function render_template_library() {
            $funnels = $this->funnel_templates;
            $cats = array_unique(array_column($funnels, 'category'));
            ?>
            <div class="lfp-template-library">
                <div class="lfp-tl-header">
                    <div><h2 class="lfp-tl-title">📚 Ready-Made Funnel Templates</h2><p class="lfp-tl-subtitle">Preview & download JSON. Then: <strong>CartFlows → Import → Upload JSON</strong></p></div>
                ...
                <div class="lfp-tl-search-bar">
                    <div class="lfp-tl-search-input-wrap"><span class="lfp-tl-search-icon">🔍</span><input type="text" class="lfp-tl-search-input" id="lfp-tl-search" placeholder="Search templates..."></div>
                    <div class="lfp-tl-filters"><button class="lfp-tl-filter-btn active" data-filter="all">🗂️ All</button><?php foreach($cats as $c): ?><button class="lfp-tl-filter-btn" data-filter="<?php echo esc_attr(sanitize_title($c)); ?>"><?php echo esc_html($c); ?></button><?php endforeach; ?></div>
                </div>
                <div class="lfp-tl-grid">
                    <?php foreach($funnels as $f): $st = $f['steps'].' '.($f['steps']>1?'Steps':'Step'); ?>
                        <div class="lfp-tl-card <?php echo $f['featured']?'lfp-tl-featured':''; ?>" data-category="<?php echo esc_attr(sanitize_title($f['category'])); ?>" data-search="<?php echo esc_attr(strtolower($f['title'].' '.implode(' ',$f['tags']))); ?>">
                            <div class="lfp-tl-card-img">
                                <img src="<?php echo esc_url($f['preview_img']); ?>" alt="<?php echo esc_attr($f['title']); ?>" loading="lazy">
                                <div class="lfp-tl-card-overlay">
                                    <button class="lfp-tl-preview-btn" data-title="<?php echo esc_attr($f['title']); ?>" data-img="<?php echo esc_url($f['preview_img']); ?>" data-steps="<?php echo esc_attr($st); ?>" data-desc="<?php echo esc_attr($f['desc']); ?>"><span>🔍</span> Quick Preview</button>
                                    <button class="lfp-tl-download-btn-overlay" data-json="<?php echo esc_url($f['json_url']); ?>" data-title="<?php echo esc_attr($f['title']); ?>"><span>📥</span> Download JSON</button>
                                </div>
                                <span class="lfp-tl-type-badge"><?php echo $f['type']==='funnel'?'🔄 Funnel':'📄 Template'; ?></span>
                                <?php if($f['featured']): ?><span class="lfp-tl-featured-badge">⭐ Featured</span><?php endif; ?>
                            </div>
                            <div class="lfp-tl-card-info">
                                <h3 class="lfp-tl-card-title"><?php echo esc_html($f['title']); ?></h3>
                                <p class="lfp-tl-card-desc"><?php echo esc_html($f['desc']); ?></p>
                                <div class="lfp-tl-card-meta">
                                    <span class="lfp-tl-meta-item"><span class="lfp-tl-meta-icon">📋</span><?php echo $st; ?></span>
                                    <span class="lfp-tl-meta-item"><span class="lfp-tl-meta-icon">🏷️</span><?php echo esc_html($f['category']); ?></span>
                                </div>
                                <div class="lfp-tl-tags"><?php foreach($f['tags'] as $t): ?><span class="lfp-tl-tag"><?php echo esc_html($t); ?></span><?php endforeach; ?></div>
                            </div>
                            <div class="lfp-tl-card-footer">
                                <button class="lfp-btn lfp-btn-primary lfp-tl-footer-btn lfp-tl-download-btn-footer" data-json="<?php echo esc_url($f['json_url']); ?>" data-title="<?php echo esc_attr($f['title']); ?>"><span>📥</span> Download JSON</button>
                                <button class="lfp-btn lfp-btn-ghost lfp-tl-footer-btn lfp-tl-preview-btn" data-title="<?php echo esc_attr($f['title']); ?>" data-img="<?php echo esc_url($f['preview_img']); ?>" data-steps="<?php echo esc_attr($st); ?>" data-desc="<?php echo esc_attr($f['desc']); ?>"><span>🔍</span> Preview</button>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
